...
var App = /*#__PURE__*/function () {
  ...
  _createClass(App, [{
    ...
    value: function load() {
      ...
      if (this._status === _consts.INIT) {
        this._status = _consts.LOADING;

        var leftAssets = this._assets.filter(function (asset) {
          return !asset.isLoaded();
        });

        Promise.all(leftAssets.map(function (asset) {
          return asset.load(parentEl);
        })).then(this._onLoad)["catch"](this._onError);
      }

      if (this._status === _consts.LOADING) {
